...
(function($){
	$.widget("ui.formwizard", {
		...
		_init: function() {
			...
			if (this.options.firstStep != undefined) {
        this.firstStep = this.options.firstStep;
      }
    ...
    setRemoteAjaxUrl : function(step, aurl) {
      this.options.remoteAjax[step].url = aurl;
      console.log("Set the step for "+step+" to "+this.options.remoteAjax[step].url);
	   	 //alert(step + "=" + url);
		},
